...
class TimeZone:
    def __init__(self, master):
        ...
        style = ttk.Style()
        style.configure("TLabel", font=("Calibri", 20), padding=20, foreground="red", background="black")
        style.configure("TButton", font=("Calibri", 20), padding=20, foreground="red", background="black")

            #! Create a dropdown menu with popular timezones
        popular_timezones = [
            "US/Pacific", "US/Mountain", "US/Central", "US/Eastern", 
            "Europe/London", "Europe/Paris","Europe/Kyiv",
            "Asia/Calcutta", "Asia/Hong_Kong", "Asia/Tokyo", "Australia/Sydney"
        ]
        self.timezone_var = tk.StringVar()
        self.timezone_var.set("UTC")
        timezone_label = ttk.Label(self.master, text="Timezone Selection :", style="TLabel")
        timezone_label.grid(row=0, column=0, padx=20, pady=20)
        timezone_menu = ttk.OptionMenu(self.master, self.timezone_var, *popular_timezones)
        ...
        update_button = ttk.Button(self.master, text="GO", command=self.update_time, style="TButton")
